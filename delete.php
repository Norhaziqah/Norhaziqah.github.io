...
<body>
	...
	//database section -- printing image,title, and contact no.
 		...
 		while($row=mysqli_fetch_assoc($result))
 	{
 	 	/*echo $row['image']; */
 	 	?>
 	<div id='info' class='image'>
 		 <p class='title'>Title:<?php echo $row['title'] ?></p>
        <form  action='' method="post">
        <input type="submit" class='yellow' name="action" id='delete' value="Delete" />
        <input type="hidden" name="id" value="<?php echo $row['id']; ?>"/>
        <input type="hidden" name="imagefile" value="<?php echo $row['image']; ?>"/>
        	<?php
      	if ($_POST['action'] && $_POST['id']) {
  			if ($_POST['action'] == 'Delete') {
  			$deleteImg = getcwd() ."/post/". $_POST['imagefile'];
  			unlink($deleteImg);
  			//unlink('./MAMP/htdocs/project2/post/'.$_POST['imagefile']);
    		$sqldel = "DELETE FROM item WHERE id =". $_POST['id'];
    		$conn->query($sqldel);

    		header("Refresh:0"); //refreshes after deleting
 		 }
	}
